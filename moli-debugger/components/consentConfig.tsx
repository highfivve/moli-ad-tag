...
const cmpVendors: CmpVendors = {
  '3': { name: ' LiveRamp', website: 'https://liveramp.com/', subDomain: '\tfaktor.mgr.consensu.org' },
  '5': { name: ' Usercentrics GmbH', website: 'https://usercentrics.com/', subDomain: 'usercentrics.mgr.consensu.org' },
  '6': { name: 'Sourcepoint Technologies, Inc.', website: 'https://sourcepoint.com', subDomain: 'sourcepoint.mgr.consensu.org' },
  '10': { name: 'Quantcast International Limited', website: 'https://quantcast.com', subDomain: 'quantcast.mgr.consensu.org' },
  '21': { name: 'Traffective GmbH', website: 'https://traffective.com', subDomain: 'traffective.mgr.consensu.org' },
};
...
export class ConsentConfig extends preact.Component<{}, IConsentConfigState> {
  ...
  private consent = (): JSX.Element => {
    ...
    return <div>
      <div className="MoliDebug-tagContainer">
        <span className="MoliDebug-tagLabel"><a href="https://iabeurope.eu/cmp-list/">CMP ID</a></span>
        {this.cmpVendorTag()}
      </div>
      <div className="MoliDebug-tagContainer">
        <span className="MoliDebug-tagLabel">Last updated</span>
        {this.state.tcModel ? <Tag>{this.state.tcModel.lastUpdated.toLocaleString()}</Tag> :
          <Tag variant="red">Unknown</Tag>}
      </div>
      <div className="MoliDebug-tagContainer">
        <span className="MoliDebug-tagLabel">TCF Version</span>
        {this.state.tcModel ? <Tag>{this.state.tcModel.version.toString()}</Tag> : <Tag variant="red">Unknown</Tag>}
      </div>
    </div>;
  };
  ...
  private cmpVendorTag = (): JSX.Element => {
    if (this.state.tcModel) {
      ...
      if (cmpVendor) {
        return <Tag variant="grey"><a href={cmpVendor.website}>{cmpVendor.name} (id: {this.state.tcModel.cmpId.toString()})</a></Tag>;
      } else {
    ...
    if (this.isCmpFunctionAvailable()) {
      ...
      return <div>
        <div class="MoliDebug-tagContainer">
          ...
          <Tag>{this.state.tcModel ? this.state.tcModel.vendorListVersion.toString() : 'not found'}</Tag>
        </div>
        <div class="MoliDebug-tagContainer">
          <span class="MoliDebug-tagLabel">num vendors allowed</span>
          <Tag>{this.state.tcModel ? this.state.tcModel.vendorConsents.size.toString() : 'not found'}</Tag>
        </div>
        <div class="MoliDebug-tagContainer">
          <span className="MoliDebug-tagLabelBtn" type="button" data-toggle="collapse"
                data-target="#collapsePurposes"
                aria-expanded="false" aria-controls="collapsePurposes">
            allowed purposes
          </span>
          <Tag>{this.state.tcModel ? this.state.tcModel.purposeConsents.size.toString() : 'not found'}</Tag>

          <div className="collapse" id="collapsePurposes">
            <a
              href="https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Appendix_A_Purposes_and_Features_Definitions">Purpose
              and Feature definitions</a>
            <br/>
            {this.state.tcModel && Array.from(this.state.tcModel.purposeConsents).map(([ id, accepted ]) => {
              return <Tag
                variant={accepted ? 'green' : 'red'}>{id.toString()}: {accepted ? 'accepted' : 'denied'}</Tag>;
            })}
          </div>
        </div>
