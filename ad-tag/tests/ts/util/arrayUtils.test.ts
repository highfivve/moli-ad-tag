...
describe('flatten', () => {
  ...
  it('should reduce a 3-dimensional array to 2 dimensions', () => {
    ...
    expect(new Set(flatArray)).to.deep.equal(new Set([ [ 200, 50 ], [ 350, 200 ] ]));
  });
});
...
describe('uniquePrimitiveFilter', () => {
  ...
  it('should be able to filter an array of strings', () => {
    const uniqueArray = [ 'same', 'same', 'but', 'different', 'but', 'still', 'the', 'same' ].filter(uniquePrimitiveFilter);
