...
export const createMoliTag = (window: Window): Moli.MoliTag => {
      ...
      switch (state.state) {
        ...
        case 'configured': {
          ...
          if (isSinglePageApp) {
            ...
            const spaRequestAdsState: ISinglePageApp = {
              ...
              config: config,
              refreshAds: (moliConfig: Moli.MoliConfig, refreshSlots: string[]) => adService.requestAds(moliConfig, refreshSlots).then(() => {
                return;
              }),
              initialized,
            ...
            return initialized
              ...
              .then(() => {
                ...
                if (state.state === 'spa-requestAds' && state.href === window.location.href) {
                  ...
                  // nothing to do here
                  getLogger(state.config, window).debug('MoliGlobal', 'A previous requestAds() was slower than the following requestAds() call');
                  return state;
                } else {
        ...
        case 'spa-finished': {
          if (state.state === 'spa-requestAds') {
            getLogger(state.config, window).debug('MoliGlobal', 'requestAds is being called while the previous requestAds() hasn\'t finished yet');
          }
          ...
          return initialized
            ...
            .then(configWithTargeting => refreshAds(configWithTargeting, refreshSlots).then(() => configWithTargeting))
    ...
    function refreshAdSlot(domId: string): Promise<'queued' | 'refreshed'> {
      switch (state.state) {
        ...
        default : {
          getLogger(state.config, window).error('MoliGlobal', `refreshAdSlot is not allowed in state ${state.state}`, state.config);
