...
export class YieldOptimizationService {
    ...
    return this.getPriceRule(adUnitDomId).then(rule => {
      if (rule) {
        this.log.debug(
          ...
          `set price rule id ${
            rule.priceRuleId
          } for ${adUnitDomId}. Main traffic share ${!!rule.main}. cpm is ${rule.cpm}`
        );
  ...
  ): Promise<PublisherYieldConfiguration> {
    if (retriesLeft <= 0) {
      return Promise.reject(lastError);
    }

    return this.assetLoaderService
      .loadJson<PublisherYieldConfiguration>('yield-config.json', configEndpoint)
      .catch(error => {
