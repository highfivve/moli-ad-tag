...
const Tag = ({ children, variant, title, spacing }) => (React.createElement("div", { className: classList('MoliDebug-tag', [!!variant, `MoliDebug-tag--${variant}`], [!!spacing, `MoliDebug-tag--${spacing}Spacing`]), title: title }, children));
const TagLabel = ({ children }) => (React.createElement("span", { className: "MoliDebug-tagLabel" }, children));

const SizeConfigDebug = ({ sizeConfig, supportedLabels }) => {
...
const extractPrebidAdSlotConfigs = (prebid) => {
...
function uniquePrimitiveFilter(elm, position, arr) {
    ...
    constructor(sizeConfig, supportedLabels, window) {
        ...
        this.filterSupportedSizes = (givenSizes) => {
            ...
            return this.supportedSizes.length === 0
                ? givenSizes
                : this.supportedSizes.filter(configuredSize => givenSizes.some(givenSize => {
...
class AdSlotConfig extends React.Component {
    constructor(props) {
        super(props);
        this.prebidConfig = (prebid) => {
            ...
            const elements = prebidAdUnits.map((prebidAdUnit, index) => {
                ...
                return (React.createElement("div", { key: index },
                    ...
                    prebidAdUnit.bids.map((bid, idx) => (React.createElement(reactExports.Fragment, { key: idx },
    ...
    render() {
        ...
        return (React.createElement("div", { className: classList('MoliDebug-adSlot', [!!parentElement, 'MoliDebug-adSlot--overlay'], [!parentElement && slotVisible, 'is-rendered'], [!parentElement && !slotVisible, 'is-notRendered'], [isConfiguredInfiniteSlot, 'is-configuredInfinite']), style: dimensions },
            React.createElement("div", { className: "MoliDebug-adSlot-buttons" },
                ...
                isConfiguredInfiniteSlot && (React.createElement("p", null, `Found ${window.document.querySelectorAll(slot.behaviour.selector).length} slots with infinite selector ${slot.behaviour.selector}`))),
            showGeneral && (React.createElement("div", { className: "MoliDebug-panel MoliDebug-panel--blue MoliDebug-panel--collapsible" },
                React.createElement("div", { className: "MoliDebug-tagContainer" },
                    ...
                    slot.behaviour.bucket && React.createElement(Tag, { variant: "blue" }, slot.behaviour.bucket)),
                React.createElement("div", { className: "MoliDebug-tagContainer" },
...
(function (tcfapi) {
    (function (responses) {
        ...
        })(responses.TCPurpose || (responses.TCPurpose = {}));
        (function (RestrictionType) {
            RestrictionType[RestrictionType["NOT_ALLOWED"] = 0] = "NOT_ALLOWED";
            RestrictionType[RestrictionType["REQUIRE_CONSENT"] = 1] = "REQUIRE_CONSENT";
            RestrictionType[RestrictionType["REQUIRE_LI"] = 2] = "REQUIRE_LI";
        })(responses.RestrictionType || (responses.RestrictionType = {}));
    })(tcfapi.responses || (tcfapi.responses = {}));
    (function (status) {
        ...
        })(status.CmpStatus || (status.CmpStatus = {}));
...
const LabelConfigDebug = ({ labelSizeConfig }) => {
...
const updateQueryString = (key, value, baseUrl, search) => {
...
const getBrowserStorageValue = (key, storage = localStorage) => {
    try {
...
const setBrowserStorageValue = (key, value, storage = localStorage) => {
    try {
...
const removeBrowserStorageValue = (key, storage = localStorage) => {
    try {
...
const getDebugDelayFromLocalStorage = (window) => Number(getBrowserStorageValue(BrowserStorageKeys.debugDelay, window.localStorage)) || undefined;
const setDebugDelayToLocalStorage = (window, value) => {
    if (value) {
...
const removeTestSlotSizeFromLocalStorage = (slot) => removeBrowserStorageValue(BrowserStorageKeys.testSlotSize(slot.domId));
...
const checkAdReloadConfig = (messages, modules, slots, labels) => {
    const module = modules.find(module => module.moduleType === 'ad-reload');
    if (!module) {
        return;
    }
    const adReloadConfig = module.config;
    slots.forEach(slot => {
        if (!slot.prebid) {
            return;
        }
...
const checkSizesConfig = (messages, slots, labels) => {
...
const checkBucketConfig = (messages, bucket, slots) => {
...
const checkSkinConfig = (messages, modules, slots) => {
    const module = modules.find(module => module.name === 'skin');
    if (module) {
        const skinModule = module.config;
...
const resolveOverrides = (window, queryParam, storageKey, predicate = (value) => true) => {
    return [
        {
            source: 'queryParam',
...
const getActiveEnvironmentOverride = (window) => resolveOverrides(window, QueryParameters.moliEnv, BrowserStorageKeys.moliEnv, isEnvironmentString)[0] || undefined;
const setEnvironmentOverrideInStorage = (value, storage) => setBrowserStorageValue(BrowserStorageKeys.moliEnv, value, storage);
const resetEnvironmentOverrides = (window) => {
    ...
    window.location.replace(updateQueryString(QueryParameters.moliEnv));
};
...
var css_248z = ".link{color:#fff;text-decoration:underline}.MoliDebug-module:before{align-items:center;background:#90be6d;border-radius:4px;color:#fff;content:\"-- \" attr(data-module-key) \" --\";display:flex;font-size:12px;font-weight:700;height:16px;justify-content:center;margin:8px 0;padding:0 4px;width:42px}.MoliDebug-info{margin-top:8px}.MoliDebug-sidebar{background:#fff;box-shadow:-3px 0 5px 0 rgba(0,0,0,.2);box-sizing:border-box;color:#242424;display:block;font-family:Helvetica,Arial,sans-serif;font-size:14px;max-height:100vh;max-width:100vw;overflow-y:auto;padding:32px 0 15px;position:fixed;right:0;text-align:left;top:0;z-index:99999999}.dark .MoliDebug-sidebar{background:#454547;color:#d9d9d9}@media (min-width:768px){.MoliDebug-sidebar{max-width:700px;min-width:500px}}.MoliDebug-sidebar.is-hidden{display:none}.MoliDebug-sidebar pre{margin:0}.MoliDebug-sidebar-closeHandle{background:#fff;border:none;box-shadow:-3px 0 5px 0 rgba(0,0,0,.2);cursor:pointer;height:32px;left:0;padding-left:10px;position:fixed;top:0;width:100vw;z-index:100000000}.dark .MoliDebug-sidebar-closeHandle{background:#ffcb00;color:#242424}@media (min-width:768px){.MoliDebug-sidebar-closeHandle{border-bottom-left-radius:4px;left:auto;right:0;width:auto}}.MoliDebug-sidebar-closeHandle:hover{background:#ccc}.MoliDebug-sidebarSection{border-left:6px solid;margin-bottom:10px;padding-left:15px}.MoliDebug-sidebarSection:last-child{margin-bottom:15px}.MoliDebug-sidebarSection--noBorder{border-left:none}.MoliDebug-sidebarSection--slots{border-left-color:#506684}.MoliDebug-sidebarSection--moli,.MoliDebug-sidebarSection--targeting{border-left-color:#998fc7}.MoliDebug-sidebarSection--sizeConfig{border-left-color:#d4c2fc}.MoliDebug-sidebarSection--prebid{border-left-color:#76949f}.MoliDebug-sidebarSection--consent{border-left-color:#d9f4c7}.MoliDebug-sidebarSection--linting{border-left-color:#f4ef88}.MoliDebug-adSlot{background:hsla(0,0%,100%,.8);box-sizing:border-box;display:flex;flex-direction:column;font-family:Helvetica,Arial,sans-serif;font-size:14px;margin:0 0 15px;min-height:34px;text-align:left}.dark .MoliDebug-adSlot{background:transparent}.MoliDebug-adSlot.is-rendered{border-left:4px solid #90be6d}.MoliDebug-adSlot.is-notRendered{border-left:4px solid #db3b3d}.MoliDebug-adSlot.is-configuredInfinite{border-left:4px solid #ffe152}.MoliDebug-adSlot--overlay{border:1px solid #000;border-radius:4px;height:100%;left:0;min-height:100px;min-width:300px;overflow-y:auto;position:absolute;top:0;width:100%;z-index:99999998}.MoliDebug-adSlot-buttons{display:flex;height:34px;min-height:34px;padding:5px 0}.MoliDebug-adSlot-button{background:#d3d3d3;border:1px solid #d3d3d3;border-radius:4px;color:#000;cursor:pointer;height:24px;line-height:20px;margin:0 5px;padding:0;width:24px}.MoliDebug-adSlot-button.is-active,.MoliDebug-adSlot-button:focus,.MoliDebug-adSlot-button:hover{background:#fff;border-color:#000}.MoliDebug-adSlot-button.is-rendered{background:#90be6d}.MoliDebug-adSlot-button.is-notRendered{background:#db3b3d}.MoliDebug-adSlot-button.is-configuredInfinite{background:#ffe152}.MoliDebug-adSlot-button--sizeConfig:before{content:\"\\01F4D0\"}.MoliDebug-panel{margin-bottom:10px;padding:5px}.MoliDebug-panel ul{margin:5px 0;padding-left:20px}.MoliDebug-panel--grey{background:#d3d3d3;border-radius:4px}.dark .MoliDebug-panel--grey{background:#6a6a6a;color:#fff}.MoliDebug-panel--red{background:#e9898b}.MoliDebug-panel--blue{background-color:#edf6fc}.dark .MoliDebug-panel--blue{color:#454547}.MoliDebug-panel--collapsible{box-shadow:3px 2px 2px 0 #76949f;margin-bottom:20px}.MoliDebug-button{border:1px solid #d3d3d3}.MoliDebug-button:hover{border-color:#000}.MoliDebug-formContainer{align-items:center;display:flex;gap:10px}.MoliDebug-button,.MoliDebug-tag{background:#76949f;border-radius:4px;color:#fff;display:inline-block;margin-right:5px;padding:1px 5px}.MoliDebug-button--red,.MoliDebug-tag--red{background:#db3b3d}.MoliDebug-button--redText,.MoliDebug-tag--redText{color:#db3b3d}.MoliDebug-button--green,.MoliDebug-tag--green{background:#90be6d}.MoliDebug-button--greenText,.MoliDebug-tag--greenText{color:#90be6d}.MoliDebug-button--greyText,.MoliDebug-tag--greyText{color:#454547}.MoliDebug-button--yellow,.MoliDebug-tag--yellow{background:#ffe152}.dark .MoliDebug-button--yellow,.dark .MoliDebug-tag--yellow{background:#ffcb00}.MoliDebug-button--blue,.MoliDebug-tag--blue{background:#76949f}.MoliDebug-button--grey,.MoliDebug-tag--grey{background:#e9ecef}.MoliDebug-button--transparent,.MoliDebug-tag--transparent{background:transparent;color:#000}.MoliDebug-button.MoliDebug-button--mediumSpacing,.MoliDebug-tag.MoliDebug-tag--mediumSpacing{margin-bottom:5px}.MoliDebug-buttonLabel,.MoliDebug-tagLabel{display:inline-block;min-width:140px;padding-right:5px}.MoliDebug-buttonLabelBtn,.MoliDebug-tagLabelBtn{background:#d3d3d3;border:1px solid #d3d3d3;border-radius:4px;cursor:pointer;display:inline-block;margin-bottom:5px;margin-right:5px;min-width:135px;padding-left:5px;padding-right:5px}.dark .MoliDebug-buttonLabelBtn,.dark .MoliDebug-tagLabelBtn{color:#454547}.MoliDebug-buttonContainer,.MoliDebug-tagContainer{margin-top:5px}.MoliDebug-buttonContainer.MoliDebug-buttonContainer--subEntry,.MoliDebug-tagContainer.MoliDebug-tagContainer--subEntry{margin-left:10px}.MoliDebug-buttonContainer.MoliDebug-buttonContainer--subEntry:before,.MoliDebug-tagContainer.MoliDebug-tagContainer--subEntry:before{content:\"\\21B3\";display:inline;margin-right:5px}.MoliDebug-keyValueTable{text-align:left}.MoliDebug-keyValueTable td,.MoliDebug-keyValueTable th{padding-right:10px}.MoliDebug-configMessage{align-items:baseline;border-left:4px solid grey;display:flex;min-height:24px;padding:6px 0}.MoliDebug-configMessage:hover{background-color:#e4f1ff}.dark .MoliDebug-configMessage:hover{color:#242424}.MoliDebug-configMessage--error{border-left-color:#db3b3d}.MoliDebug-configMessage--warning{border-left-color:#ffe152}.MoliDebug-configMessage--empty{border-left-color:#90be6d}.MoliDebug-configMessage-icon{flex:0 0 32px;text-align:center}.MoliDebug-checkBox{-webkit-user-select:none;-moz-user-select:none;user-select:none}.MoliDebug-posRelative{position:relative}.size-valid-table td,.size-valid-table th{border:1px solid;padding:10px;text-align:center}.size-valid-table th{background:#d9d9d9}";
styleInject(css_248z);

const debugSidebarSelector = 'moli-debug-sidebar';
class GlobalConfig extends reactExports.Component {
    constructor(props) {
        super(props);
        this.listener = () => {
        ...
        this.unwrapConfig = (moduleConfig, subEntry = false) => {
            return (React.createElement(reactExports.Fragment, null, Object.keys(moduleConfig).map((key, index) => {
    ...
    extractAdVersionFromS2sConfig(s2sConfig) {
        if (!s2sConfig) {
            return '?';
        }
        if (Array.isArray(s2sConfig)) {
            return (s2sConfig[0]?.extPrebid?.analytics?.h5v.adTagVersion ?? '?');
        }
        else {
            return s2sConfig.extPrebid?.analytics?.h5v.adTagVersion ?? '?';
        }
    }
    ...
    render() {
        ...
        const { sidebarHidden, showOnlyRenderedSlots, expandSection, theme, adstxtEntry, adstxtDomain } = this.state;
        ...
        return (React.createElement("div", { id: "moli-console-global-config" },
            React.createElement("style", null, css_248z),
            ...
            config && (React.createElement("div", { className: classes, "data-ref": debugSidebarSelector },
                React.createElement("div", { className: "MoliDebug-sidebarSection  MoliDebug-sidebarSection--moli" },
                    React.createElement("div", { className: "MoliDebug-tagContainer" },
                        React.createElement("div", { className: "MoliDebug-tagContainer" },
                            ...
                            React.createElement(Tag, null, adTagVersion)),
                        React.createElement("br", null),
                    ...
                    expandSection.moli && (React.createElement("div", null,
                        React.createElement("div", { className: "MoliDebug-tagContainer" },
                            React.createElement(TagLabel, null, "Mode"),
                            ...
                            isEnvironmentOverriden ? (React.createElement("button", { className: "MoliDebug-button MoliDebug-button--green", onClick: this.resetEnvironmentOverrides }, "\u25C0 Reset override")) : (React.createElement("button", { className: "MoliDebug-button MoliDebug-button--yellow MoliDebug-button--greyText", onClick: this.overrideEnvironmentToTest }, "\u25B6 Override to test"))),
                        React.createElement("div", { className: "MoliDebug-tagContainer" },
                        ...
                        React.createElement("div", { className: "MoliDebug-tagContainer" },
                            ...
                            React.createElement("br", null),
                            React.createElement("br", null)),
                        modules.length > 0 && (React.createElement(React.Fragment, null,
                            React.createElement("h5", null,
                                this.collapseToggle('modules'),
                                "Moli Modules"),
                            expandSection.modules && (React.createElement(React.Fragment, null, modules.map((module, index) => {
                                const moduleConfig = module.config;
                                return (React.createElement("div", { key: index },
                                    React.createElement("div", { className: "MoliDebug-tagContainer MoliDebug-module", "data-module-key": index + 1 },
                                        React.createElement(Tag, null, module.name)),
                                    React.createElement("div", { className: "MoliDebug-tagContainer" },
                                        React.createElement(TagLabel, null, "Module Description"),
                                        React.createElement(Tag, { variant: "transparent" }, module.description)),
                                    React.createElement("div", { className: "MoliDebug-tagContainer" },
                                        React.createElement(TagLabel, null, "Module Type"),
                                        React.createElement(Tag, { variant: "blue" }, module.moduleType)),
                                    moduleConfig && (React.createElement(reactExports.Fragment, null,
                                        React.createElement("h6", null, "Module Config"),
                                        this.unwrapConfig(moduleConfig))),
                                    index !== modules.length - 1 && React.createElement("hr", null)));
                            })))))))),
                ...
                config.prebid && (React.createElement("div", { className: "MoliDebug-sidebarSection MoliDebug-sidebarSection--prebid" },
                    ...
                    expandSection.prebid && (React.createElement("div", null,
                        ...
                        React.createElement("div", { className: "MoliDebug-tagContainer" },
                            ...
                            React.createElement(Tag, null, config.prebid.config.currency.adServerCurrency)),
                        React.createElement("div", { className: "MoliDebug-tagContainer" },
                ...
                React.createElement("div", { className: "MoliDebug-sidebarSection MoliDebug-sidebarSection--linting" },
                    React.createElement("h4", null, "Moli configuration issues and warnings"),
                    this.state.messages.map((message, index) => (React.createElement("div", { key: `${message.text}-${index}`, className: classList('MoliDebug-configMessage', `MoliDebug-configMessage--${message.kind}`) },
                        this.iconForMessageKind(message.kind),
                        message.text))),
                    this.state.messages.length === 0 && (React.createElement("div", { className: classList('MoliDebug-configMessage', `MoliDebug-configMessage--empty`) },
...
new WindowResizeService();

class LabelConfigService {
    constructor(labelSizeConfig, extraLabels, window) {
        this.labelSizeConfig = labelSizeConfig;
        this.extraLabels = extraLabels;
        this.window = window;
        this.possibleDevices = ['mobile', 'desktop', 'android', 'ios'];
        const isPublisherDeviceDefined = extraLabels.some((label) => this.possibleDevices.indexOf(label) > -1);
    ...
    getDeviceLabel() {
        return (this.getSupportedLabels().find((label) => this.possibleDevices.indexOf(label) > -1) || 'mobile');
    }
}

const moliConfig = window.moli.getConfig();
const modulesMeta = 'getModuleMeta' in window.moli ? window.moli.getModuleMeta() : [];
if (moliConfig) {
    ...
    const globalConfigComponent = (React.createElement(GlobalConfig, { config: moliConfig, runtimeConfig: window.moli.getRuntimeConfig(), modules: modulesMeta, labelConfigService: labelConfigService, windowResizeService: new WindowResizeService(), themingService: themingService }));
